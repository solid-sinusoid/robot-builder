...
from robot_builder.writer.gazebo import GazeboUrdfWriter
from robot_builder.writer.ros2_control import Ros2ControlWriter
...
class URDF_writer(RobotVisitor):
    def __init__(self, filename_handler=None, mesh_dir: str = "") -> None:
    ...
    def visit_robot(self, config: etree._Element | dict, element: Robot):
        ...
        for link in element.links:
            link.visit(config, self)

        for joint in element.joints:
            joint.visit(config, self)

        if element.control:
            r2c_visistor = Ros2ControlWriter()
            for control in element.control:
                control.visit(config, r2c_visistor)

        if element.gazebo:
            gazebo_visitor = GazeboUrdfWriter()
            for gazebo in element.gazebo:
                gazebo.visit(config, gazebo_visitor)
    ...
    def visit_inertial(self, config: etree._Element | dict, element: Inertial):
        ...
        etree.SubElement(xml_child, "mass", attrib={"value": str(element.mass)})
        self._writing(xml_child, element)
    ...
    def visit_axis(self, config: etree._Element | dict, element: Axis):
        ...
        if element.axis is not None:
            etree.SubElement(
                config, "axis", attrib={"xyz": " ".join(map(str, element.axis))}
            )
    ...
    def visit_calibration(self, config: etree._Element | dict, element: Calibration):
        ...
        etree.SubElement(
            config,
            "calibration",
            attrib={"rising": str(element.rising), "falling": str(element.falling)},
        )
    ...
    def visit_sphere(self, config: etree._Element | dict, element: Sphere):
        if isinstance(config, dict):
            return NotImplemented
        etree.SubElement(config, "sphere", attrib={"radius": str(element.radius)})
    ...
    def visit_cylinder(self, config: etree._Element | dict, element: Cylinder):
        ...
        etree.SubElement(
            config,
            "cylinder",
            attrib={"radius": str(element.radius), "length": str(element.length)},
        )
    ...
    def visit_box(self, config: etree._Element | dict, element: Box):
        ...
        if element.size is not None:
            etree.SubElement(
                config, "box", attrib={"size": " ".join(map(str, element.size))}
            )

    def visit_mesh(self, config: etree._Element | dict, element: Mesh):
        if isinstance(config, dict):
            return NotImplemented
        xml_child = etree.SubElement(
            config,
            "mesh",
            attrib={"filename": self._filename_handler(element.filename)},
        )
        self._writing(xml_child, element)
    ...
    def visit_color(self, config: etree._Element | dict, element: Color):
        ...
        if element.rgba is not None:
            etree.SubElement(
                config, "color", attrib={"rgba": " ".join(map(str, element.rgba))}
            )
    ...
    def visit_texture(self, config: etree._Element | dict, element: Texture):
        ...
        if element.filename is not None:
            etree.SubElement(config, "texture", attrib={"filename": element.filename})
